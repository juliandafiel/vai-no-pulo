...
export class MailService {
    ...
    async sendWelcomeEmail(email: string, name: string, userType: 'USER' | 'DRIVER') {
        try {
    ...
    async sendProfileApprovedEmail(email: string, name: string) {
        try {
            const mailOptions = {
                from: `"Vai no Pulo" <${process.env.GMAIL_USER}>`,
                to: email,
    ...
    async sendProfileRejectedEmail(email: string, name: string, reason: string) {
        try {
            const mailOptions = {
                from: `"Vai no Pulo" <${process.env.GMAIL_USER}>`,
                to: email,
    ...
    async sendVehicleChangeApprovedEmail(email: string, name: string, vehicleModel: string, vehiclePlate: string, adminMessage?: string) {
        try {
    ...
    async sendVehicleChangeRejectedEmail(email: string, name: string, vehicleModel: string, vehiclePlate: string, reason: string) {
        try {
            const mailOptions = {
                from: `"Vai no Pulo" <${process.env.GMAIL_USER}>`,
                to: email,
    ...
    async sendVehicleApprovedEmail(email: string, name: string, vehicleModel: string, vehiclePlate: string, adminMessage?: string) {
        try {
            const messageSection = adminMessage
                ? `
                    <div style="background-color: #e8f4fd; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #667eea;">
    ...
    async sendVehicleRejectedEmail(email: string, name: string, vehicleModel: string, vehiclePlate: string, reason: string) {
        try {
            const mailOptions = {
                from: `"Vai no Pulo" <${process.env.GMAIL_USER}>`,
                to: email,
