...
describe('OrdersService', () => {
    ...
    describe('create', () => {
        ...
        it('should throw NotFoundException if trip not found', async () => {
            ...
            await expect(service.create({ tripId: 'invalid', description: 'Test', weight: 5, estimatedPrice: 50 }, 'customer-1')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw BadRequestException if customer is the driver', async () => {
            const mockTrip = { id: 'trip-1', driverId: 'driver-1' };
            mockPrismaService.trip.findUnique.mockResolvedValue(mockTrip);
            await expect(service.create({ tripId: 'trip-1', description: 'Test', weight: 5, estimatedPrice: 50 }, 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw BadRequestException if customer already has pending order', async () => {
            ...
            await expect(service.create({ tripId: 'trip-1', description: 'Test', weight: 5, estimatedPrice: 50 }, 'customer-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException when order not found', async () => {
            ...
            await expect(service.findOne('999', 'customer-1')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw ForbiddenException if user not authorized', async () => {
            ...
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            await expect(service.findOne('1', 'other-user')).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('accept', () => {
        it('should accept a pending order', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'ACCEPTED', acceptedAt: new Date() });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
        ...
        it('should throw ForbiddenException if not the driver', async () => {
            ...
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            await expect(service.accept('1', 'other-driver')).rejects.toThrow(common_1.ForbiddenException);
        });
        it('should throw BadRequestException if order not pending', async () => {
            const mockOrder = { id: '1', driverId: 'driver-1', status: 'ACCEPTED' };
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            await expect(service.accept('1', 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('reject', () => {
        it('should reject a pending order', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'REJECTED' });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            mockPrismaService.order.update.mockResolvedValue(updatedOrder);
    ...
    describe('cancel', () => {
        it('should cancel a pending order by customer', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'CANCELLED' });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            mockPrismaService.order.update.mockResolvedValue(updatedOrder);
        ...
        it('should throw BadRequestException if order in progress', async () => {
            ...
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            await expect(service.cancel('1', 'customer-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('startProgress', () => {
        it('should start progress on accepted order', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'IN_PROGRESS' });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            mockPrismaService.order.update.mockResolvedValue(updatedOrder);
    ...
    describe('complete', () => {
        it('should complete an in-progress order', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'COMPLETED', finalPrice: 50 });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            mockPrismaService.order.update.mockResolvedValue(updatedOrder);
        ...
        it('should complete with custom final price', async () => {
            ...
            const updatedOrder = Object.assign(Object.assign({}, mockOrder), { status: 'COMPLETED', finalPrice: 60 });
            mockPrismaService.order.findUnique.mockResolvedValue(mockOrder);
            mockPrismaService.order.update.mockResolvedValue(updatedOrder);
