...
describe('TripsService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: routes_service_1.RoutesService, useValue: mockRoutesService },
            ],
        }).compile();
    ...
    describe('create', () => {
        it('should create a new trip with provided vehicleId', async () => {
            ...
            mockPrismaService.trip.create.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), createData));
        ...
        it('should throw error if no approved vehicle and no pending vehicle', async () => {
            ...
            await expect(service.create(createData, 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('findAll', () => {
        ...
        it('should filter by status', async () => {
            ...
            expect(mockPrismaService.trip.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('updateStatus', () => {
        it('should update trip status to ACTIVE', async () => {
            mockPrismaService.trip.update.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'ACTIVE' }));
            const result = await service.updateStatus('1', 'ACTIVE');
            expect(result.status).toBe('ACTIVE');
        });
        it('should update trip status to COMPLETED', async () => {
            mockPrismaService.trip.update.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'COMPLETED' }));
            const result = await service.updateStatus('1', 'COMPLETED');
            expect(result.status).toBe('COMPLETED');
        });
        it('should update trip status to CANCELLED', async () => {
            mockPrismaService.trip.update.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'CANCELLED' }));
    ...
    describe('updateLocation', () => {
        it('should update trip location', async () => {
            mockPrismaService.trip.update.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { lastLocation: { lat: -23.0, lng: -46.5, ts: expect.any(Date) } }));
    ...
    describe('startTrip', () => {
        ...
        it('should throw error if not the driver', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(mockTrip);
            await expect(service.startTrip('1', 'other-driver')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw error if trip not scheduled', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'ACTIVE' }));
            await expect(service.startTrip('1', 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('completeTrip', () => {
        ...
        it('should throw error if trip not active', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(mockTrip);
            await expect(service.completeTrip('1', 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('cancelTrip', () => {
        ...
        it('should throw error if trip already completed', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'COMPLETED' }));
            await expect(service.cancelTrip('1', 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('delete', () => {
        ...
        it('should throw error if trip is active', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(Object.assign(Object.assign({}, mockTrip), { status: 'ACTIVE' }));
            await expect(service.delete('1', 'driver-1')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw error if not the driver', async () => {
            mockPrismaService.trip.findUnique.mockResolvedValue(mockTrip);
            await expect(service.delete('1', 'other-driver')).rejects.toThrow(common_1.BadRequestException);
        });
    });
});
